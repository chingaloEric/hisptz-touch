...
export class LoginPage implements OnInit {
  ...
  ngOnInit() {
    ...
    this.user.getCurrentUser().then((user: any) => {
      if (user) {
        ...
        this.loginData.serverUrl = 'https://dhis.moh.go.tz';
      }
  ...
  reInitiateProgressTrackerObject(user) {
    if (
      ...
      this.progressTracker = this.loginData.progressTracker[
        user.currentDatabase
      ];
    } else {
      this.loginData['progressTracker'] = {};
  ...
  updateProgressTracker(resourceName) {
    ...
    this.loginData['progressTracker'][
      this.loginData.currentDatabase
    ] = this.progressTracker;
  ...
  login() {
    if (this.loginData.serverUrl) {
      if (!this.loginData.username) {
        ...
        //correct server url for those redirect call
        const sanitizedUrl = this.loginData.serverUrl.split('/dhis-web')[0];
        this.loginData.serverUrl = sanitizedUrl;
        this.app
          .getFormattedBaseUrl(this.loginData.serverUrl)
          .then(formattedBaseUrl => {
            this.loginData.serverUrl = formattedBaseUrl;
            this.user.authenticateUser(this.loginData).then(
              (response: any) => {
                ...
                this.loginData.authorizationKey = btoa(
                ...
                if (!this.isLoginProcessCancelled) {
                  this.user
                    ...
                    .then(userData => {
                      this.app
                        .getDataBaseName(this.loginData.serverUrl)
                        .then(databaseName => {
                          //update authenticate  process
                          databaseName =
                            databaseName + '_' + this.loginData.username;
                          ...
                          this.sqlLite.generateTables(databaseName).then(
                            () => {
                              ...
                              if (!this.isLoginProcessCancelled) {
                                this.httpClient
                                  .get('/api/system/info', this.loginData)
                                  .subscribe(
                                    data => {
                                      data = data.json();
                                      this.updateProgressTracker(resource);
                                      ...
                                      if (!this.isLoginProcessCancelled) {
                                        this.user
                                          .setCurrentUserSystemInformation(data)
                                          .then(
                                            dhisVersion => {
                                              ...
                                              this.user
                                                ...
                                                .then(
                                                  (response: any) => {
                                                    this.loginData.authorities =
                                                      response.authorities;
                                                    if (
                                                      !this
                                                        .isLoginProcessCancelled
                                                    ) {
                                            ...
                                            error => {
                                              ...
                                              if (
                                                !this.isLoginProcessCancelled
                                              ) {
                                                this.setLoadingMessages(
                                                  'Fail to set system information'
                                                );
                                              }
                                            }
                                          );
  ...
  downloadingOrganisationUnits(userData) {
    if (!this.isLoginProcessCancelled) {
      ...
      this.progressTracker[this.currentResourceType].message =
        'Loading assigned organisation unit';
      let orgUnitIds = [];
      ...
      if (this.completedTrackedProcess.indexOf(resource) > -1) {
        this.progressTracker[this.currentResourceType].message =
          'Assigned organisation unit(s) have been loaded';
        this.updateProgressTracker(resource);
        this.downloadingDataSets();
      } else {
        this.OrganisationUnit.downloadingOrganisationUnitsFromServer(
          orgUnitIds,
          this.loginData
        ).then(
          (orgUnits: any) => {
            if (!this.isLoginProcessCancelled) {
              this.progressTracker[this.currentResourceType].message =
                'Saving assigned organisation unit(s)';
              this.OrganisationUnit.savingOrganisationUnitsFromServer(
                orgUnits,
                this.loginData
              ).then(
                () => {
                  this.progressTracker[this.currentResourceType].message =
                    'Assigned organisation unit(s) have been saved';
                  this.updateProgressTracker(resource);
                  this.downloadingDataSets();
                },
                error => {
  ...
  downloadingDataSets() {
    if (!this.isLoginProcessCancelled) {
      ...
      if (this.completedTrackedProcess.indexOf(resource) > -1) {
        ...
        this.DataSets.downloadDataSetsFromServer(this.loginData).then(
          (dataSets: any) => {
            if (!this.isLoginProcessCancelled) {
              this.progressTracker[this.currentResourceType].message =
                'Saving  entry forms';
              this.DataSets.saveDataSetsFromServer(
                dataSets,
                this.loginData
              ).then(
                () => {
  ...
  downloadingSections() {
    if (!this.isLoginProcessCancelled) {
      ...
      this.currentResourceType = 'entryForm';
      this.progressTracker[this.currentResourceType].message =
        "Loading entry forms's sections";
      if (this.completedTrackedProcess.indexOf(resource) > -1) {
        this.progressTracker[this.currentResourceType].message =
          "Entry forms's sections have been loaded";
        this.updateProgressTracker(resource);
        ...
        this.app
          .downloadMetadata(this.loginData, resource, null, fields, null)
          .then(
            response => {
              if (!this.isLoginProcessCancelled) {
                this.progressTracker[this.currentResourceType].message =
                  "Saving entry forms's sections";
                this.app
                  .saveMetadata(
                    resource,
                    response[resource],
                  ...
                  .then(
                    () => {
                      this.progressTracker[this.currentResourceType].message =
                        "Entry forms's sections have been saved";
                      this.updateProgressTracker(resource);
                      this.downloadingSmsCommand();
                    },
                    error => {
                      ...
                      if (!this.isLoginProcessCancelled) {
                        this.setToasterMessage(
                          'Fail to save data entry form sections.'
                        );
                      }
                    }
                  );
            ...
            error => {
              ...
              if (!this.isLoginProcessCancelled) {
                this.setToasterMessage(
                  'Fail to load data entry form sections.'
                );
              }
            }
          );
  ...
  downloadingSmsCommand() {
    if (!this.isLoginProcessCancelled) {
      ...
      if (this.completedTrackedProcess.indexOf(resource) > -1) {
        ...
        this.SmsCommand.getSmsCommandFromServer(this.loginData).then(
          (response: any) => {
            if (!this.isLoginProcessCancelled) {
              this.progressTracker[this.currentResourceType].message =
                'Saving SMS commands';
              this.SmsCommand.savingSmsCommand(
                response,
                this.loginData.currentDatabase
              ).then(
                () => {
                ...
                error => {
                  ...
                  this.isLoginProcessActive = false;
                  this.setToasterMessage(
                    'Fail to save metadata for send data via sms'
                  );
                }
              );
            }
          ...
          (error: any) => {
            ...
            if (!this.isLoginProcessCancelled) {
              this.setToasterMessage(
                'Fail to load metadata for send data via sms'
              );
            }
          }
        );
  ...
  downloadingProgramStageSections() {
    if (!this.isLoginProcessCancelled) {
      ...
      this.currentResourceType = 'event';
      this.progressTracker[this.currentResourceType].message =
        "Loading program stage's sections";
      if (this.completedTrackedProcess.indexOf(resource) > -1) {
        this.progressTracker[this.currentResourceType].message =
          "Program stage's sections have been loaded";
        this.updateProgressTracker(resource);
        ...
        this.app
          ...
          .then(
            response => {
              if (!this.isLoginProcessCancelled) {
                this.progressTracker[this.currentResourceType].message =
                  "Saving program stage's sections";
                this.app
                  .saveMetadata(
                    resource,
                    response[resource],
                  ...
                  .then(
                    () => {
                      this.progressTracker[this.currentResourceType].message =
                        "Program stage's sections have been saved";
                      this.updateProgressTracker(resource);
                      this.downloadingReports();
                      //this.downloadingProgramStageDataElements();
                    },
  ...
  downloadingProgramStageDataElements() {
    if (!this.isLoginProcessCancelled) {
      ...
      this.currentResourceType = 'event';
      this.progressTracker[this.currentResourceType].message =
        'Loading programstage data elements';
      if (this.completedTrackedProcess.indexOf(resource) > -1) {
        this.progressTracker[this.currentResourceType].message =
          'Programstage data elements have been loaded';
        this.updateProgressTracker(resource);
        this.downloadingReports();
      } else {
        let tableMetadata = this.sqlLite.getDataBaseStructure()[resource];
        ...
        this.app
          ...
          .then(
            response => {
              if (!this.isLoginProcessCancelled) {
                this.progressTracker[this.currentResourceType].message =
                  'Saving programstage data elements';
                this.app
                  .saveMetadata(
                    resource,
                    response[resource],
                  ...
                  .then(
                    () => {
                      this.progressTracker[this.currentResourceType].message =
                        'Programstage data elements have been saved';
                      this.updateProgressTracker(resource);
                      this.downloadingReports();
                    },
                    error => {
                      ...
                      this.isLoginProcessActive = false;
                      this.setToasterMessage(
                        'Fail to save program-stage data-elements.'
                      );
                    }
                  );
              }
            ...
            error => {
              ...
              if (!this.isLoginProcessCancelled) {
                this.setToasterMessage(
                  'Fail to load program-stage data-elements.'
                );
              }
            }
          );
  ...
  downloadingReports() {
    if (!this.isLoginProcessCancelled) {
      ...
      if (this.completedTrackedProcess.indexOf(resource) > -1) {
        ...
        this.app
          .downloadMetadata(this.loginData, resource, null, fields, filter)
          .then(
            response => {
              if (!this.isLoginProcessCancelled) {
                this.progressTracker[this.currentResourceType].message =
